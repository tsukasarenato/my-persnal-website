    ...
    [
        {
            ...
            paragraphs: [
                {p: `In this article, I'll show you how to use the Youtube API to collect videos and comments 
                data.`},
                {p: `Nowadays, it's common to see analyzes with Twitter data, since the Folha SP's ideological 
                GPS to the Politoscope project in France . I also wrote about it for the EBICC, cognitive 
                science event 2019 at Unicamp.`},
                {p: `My experience in the field is more related the politics, but you can find many uses for 
                ...
                {p: `An interesting point about this subject, which also is part of the goals of the Politoscope,
                ...
                about this subject.`},
                {p: `Before start, I need to highlight some limitations, in social media exists many paid 
                ...
                {p: `Another important thing is to be beware of the methodology used because may exist other 
                ...
                {p: `For example, in a situation of tragedy is expected that most parts of the sentiment be 
                ...
                {p: `In other words, the sentiment analysis doesn't consider the content of the message, so users with 
                different opinions can be grouped into a unique sentiment.`},
                {p: `If you want to read more about the challenges and difficulties of analyzing social media, you can 
                read Renee Boucher Ferguson's article.`},
            ]
        },
        {
            subtitle: 'Generating credentials to access the API',
            paragraphs: [
                {p: `Now, the first thing to do is access Google's developer console , and log in using your 
                Gmail, if it's your first time you will need to accept the terms and conditions.`},
                {p: `Create a project by clicking on the New Project button, in this tutorial, we'll use the 
                ...
                {p: `Now, we'll active the API to access the Youtube Data API v3 service, click on dashboard, 
                ...
                {p: `You'll need to download the API's library on your machine, to do it types:`},
                {code: codes[0]},
                {p: `You're now ready to use the API, create a file, and import the library in your code, types:`},
                {code: codes[1]},
            ]
        },
        {
            ...
            paragraphs: [
                {p: `Type and observe the following code:`},
                {code: codes[3]},
                {p: `This function receives two parameters, the first is the word used by API to search for 
                ...
                {p: `This function returns many data about the video found, if you want only some specific 
                information about the video you can set the parameter called 'part', access the 
                documentation, and look at the properties name returned by API to choose a valid value for 
                this parameter. In this case, the snippet property returns us the information that we need, 
                like title and video id.`},
                {p: `The publishedAfter and the publishedBefore parameters define the period, in this case, 
                ...
                {p: `The maxResults parameter defines the maximum number of data that can be returned by the 
                ...
                {p: `For each video that it returns, the data are following format:`},
                {code: codes[4]},
                {p: `Now, we'll select the information that we want and put it in a JSON file.`},
                {code: codes[5]},
                {p: `This is a function very simple, it selects the field names that have the information we 
                want and stores it in a string, after it we put the code in a loop to go through the list of 
                50 videos returned by the API.`},
                {p: `After implementing the functions, we'll write a code to call them.`},
                {code: codes[6]},
                {p: `For the purpose of demonstration, I chose three simple words about things that I like 
                as keywords.`},
                {p: `The code is available on my Github, so far we collect the videos data, in the next, we'll 
    ...
    [
        {
            subtitle: 'Introdução',
            paragraphs: [
                {p: `Neste artigo, mostrarei como usar a API do YouTube para coletar dados de vídeos e 
                comentários.`},
                {p: `Hoje em dia, é comum ver análises com dados do Twitter, desde o GPS ideológico da Folha 
                SP ao projeto Politoscope na França . Eu também escrevi sobre isso para o EBICC, evento de 
                ciências cognitivas 2019 na Unicamp.`},
                {p: `Minha experiência na área está mais relacionada à política, mas você pode encontrar 
                ...
                2018.`},
                {p: `Um ponto interessante sobre esse assunto, que também faz parte dos objetivos do 
                ...
                {p: `Antes de começar, preciso destacar algumas limitações, nas mídias sociais existem muitos 
                ...
                {p: `Outra coisa importante é ficar atento à metodologia utilizada porque podem existir 
                ...
                {p: `Em uma situação de tragédia por exemplo é esperado que a maior parte do sentimento seja 
                ...
                {p: `Em outras palavras, a análise de sentimento não leva em consideração o conteúdo da 
                mensagem, então usuários com opiniões diferentes podem ser agrupados em um sentimento 
                único.`},
                {p: `Se você quiser ler mais sobre os desafios e dificuldades da análise de mídia social, 
                você pode ler o artigo de Renee Boucher Ferguson.`},
            ]
        },
        {
            subtitle: 'Gerando credenciais para acessar a API',
            paragraphs: [
                {p: `Agora, a primeira coisa a fazer é acessar o console do desenvolvedor do Google , e 
                fazer login usando o seu Gmail. Se for a primeira vez, você precisará aceitar os termos e 
                condições.`},
                {p: `Crie um projeto clicando no botão Novo Projeto, neste tutorial usaremos as informações 
                ...
                {p: `Depois de criar a chave, vá para o dashboard (painel) e clique em enable APIs and 
                ...
                {p: `Você precisará baixar a biblioteca da API em sua máquina, para fazer isso digite no 
                terminal:`},
                {code: codes[0]},
                {p: `Agora você está pronto para usar a API, crie um arquivo e importar a biblioteca em seu 
                código, digite:`},
                {code: codes[1]},
            ]
        },
        {
            ...
            paragraphs: [
                {p: `Digite e observe o seguinte código:`},
                {code: codes[3]},
                {p: `Esta função recebe dois parâmetros, o primeiro é a palavra usada pela API para busca de 
                ...
                {p: `Esta função retorna muitos dados sobre o vídeo encontrado, se você quiser apenas 
                algumas informações específicas sobre o vídeo você pode definir o parâmetro chamado 'parte', 
                acesse a documentação, e olhe o nome das propriedades retornadas pela API para escolher um 
                ...
                {p: `Os parâmetros publishedAfter e publishedBefore definem o período, neste caso, 
                ...
                {p: `O parâmetro maxResults define o número máximo de dados que podem ser retornados pela 
                ...
                {p: `Para cada vídeo que ele retorna, os dados estão no seguinte formato:`},
                {code: codes[4]},
                {p: `Agora, vamos selecionar as informações que queremos e colocá-las em um arquivo JSON.`},
                {code: codes[5]},
                {p: `Esta é uma função muito simples, ela seleciona os nomes dos campos que possuem as 
                ...
                {p: `Depois de implementar as funções, escreveremos um código para chamá-las.`},
                {code: codes[6]},
                {p: `Para fins de demonstração, escolhi três palavras simples sobre coisas que gosto como 
                palavras-chave.`},
                {p: `O código está disponível no meu Github, até agora coletamos os dados dos vídeos, na 
